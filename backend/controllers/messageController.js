...
//         const receiverObjectId = new mongoose.Types.ObjectId(receiverId);
//         const senderObjectId = sender._id;
//         const senderModel = req.doctor ? 'doctor' : 'user'; // Исправляем
//         const receiver = await userModel.findById(receiverObjectId) || await doctorModel.findById(receiverObjectId);
//         if (!receiver) {
//             return res.status(404).json({ success: false, message: 'Receiver not found' });
//         }
//         const receiverModel = (await userModel.findById(receiverObjectId)) ? 'user' : 'doctor'; // Исправляем
...
//         const messages = await messageModel.find({ conversationId: conversationObjectId })
//             .populate('sender receiver', 'name email -_id');
//         res.status(200).json({ success: true, messages });
//     } catch (error) {
//         console.log('Error in getMessages:', error.message);
//         res.status(500).json({ success: false, message: 'Error fetching messages' });
